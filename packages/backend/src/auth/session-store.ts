...
/**
 * Server-side session storage. The Redis implementation (`RedisSessionStore`)
 * lives alongside this; an in-memory implementation exists for tests / a
 * Redis-less dev fallback.
 *
 * Moved here (task 2.5, from `apps/api/src/common/auth/session-store.ts`).
 */
...
/** DI token — the concrete store is bound by the consuming runtime. */
