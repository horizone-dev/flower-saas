...
- `apps/worker`: a `ProcessorRegistry` — `register(queue, handler)` + `start()`
  ...
  `INFRA_QUEUES` (`probe`, `dead-letter`; Task 2.8 adds `stream-retention`).
  Only `probe` has a registered processor in this task. This `dead-letter` queue
  is the landing zone for a **BullMQ job** that exhausts its retries — unrelated
  to the outbox dispatcher (2.4), which is a dedicated loop, not a BullMQ queue,
  and has **no** dead-letter table (see §2.4 / §7). The **outbox dispatcher
  (2.4) attaches here** later as a dedicated loop.
- `apps/scheduler`: the `REPEATABLE_JOBS` registry — enqueue only
  ...
  backlog. **Was "core, remaining, tracked before 2.8" — now DELIVERED in Task
  2.8:** the ~24h time-based `XTRIM` retention job (`stream-retention` schedule
  → worker sweep) and the outbox-lag observability (bounded aggregate metrics +
  threshold log lines). A 2026-09-04 remediation review had confirmed neither
  shipped earlier; Task 2.8 landed both.
...
`XADD` failure: `attempts++`, `available_at = now() + backoff` (bounded,
`{ baseMs: 1000, maxMs: 300000 }`), `last_error` set — **the row is retried
forever, never discarded**. A backed-off row is excluded from its own tenant's
`ORDER BY seq LIMIT 1` selection until `available_at`, so a single failing row
never blocks its tenant's later rows, and per-tenant advisory locking +
per-tenant `try/catch` mean a failing tenant never stalls another.
**No `outbox_dead_letter` table** — Task 2.4's accepted design (2026-09-04):
persistent failure is surfaced by `attempts` / `last_error` on the row plus the
lag-monitoring metrics, not by moving the row elsewhere. (Reconciliation
recorded in `PHASE-2-CORE-RESULTS.md` and §7.)

Retention (**delivered in Task 2.8**): a scheduler job (`stream-retention`
queue, ~1h cadence, `STREAM_RETENTION_SWEEP_INTERVAL_MS`) enqueues a sweep the
worker runs — `XTRIM rt:stream:{tenantId} MINID ~ <now − retentionMs>` per
tenant stream (`STREAM_RETENTION_MS`, default ≈ 24h). **Time-based, never
`MAXLEN`.** Idempotent, per-tenant independent, never touches `rt:live:*`.
...
- persistent `XADD` failure → `attempts` climbs, `available_at` backs off (bounded),
  the failing row is skipped in selection while backed off so its tenant's other
  rows still flow, and a failing tenant never stalls another; the row is **never
  discarded** (no dead-letter table) and stays visible via `attempts` /
  `last_error` + the lag metrics.
- `XTRIM` is time-based (Task 2.8) — an entry older than the window is trimmed, a
  newer one is not; the window is never a `MAXLEN`; the sweep is idempotent and
  per-tenant independent.
...
**Scope expanded by owner (2026-09-05), 13 locked decisions.** Beyond a pure
verification pass, Task 2.8 also **delivers**:

- **Realtime Stream retention** (`XTRIM … MINID`, time-based, ~24h target,
  configurable): a `stream-retention` scheduler queue → worker sweep. Idempotent,
  per-tenant independent, never touches `rt:live:*`. 8 required proofs (see §5).
- **Outbox lag / operational visibility**: bounded aggregate metrics on the
  worker `/metrics` (undispatched count, oldest-undispatched age, publish
  failures, stream growth) + threshold `logger.warn` lines. **No unbounded
  per-tenant metric series** — a breach names the worst tenant in a structured
  log line only.
- **HG-OUTBOX wording reconciliation** — no `outbox_dead_letter` table; this doc
  is reconciled to Task 2.4's accepted behaviour (§2.4 / §4 / §7).
- **CI `realtime` job** — build-blocking, least-privilege permissions; the full
  `apps/realtime` acceptance suite + `apps/worker` realtime-relay + retention.
- **`REALTIME-PROTOCOL-INPUTS.md`** — authority banner (ADR-0017 governs).

Run the full core verification (see §5). Reconfirm the cross-tenant probe suite
covers every Phase 2-core HTTP endpoint (the two localization reads; the realtime
protocol is WebSocket — no REST resume endpoint). `security-review` skill,
cumulatively `phase-1-complete` → the Task 2.8 HEAD. Tenant + branch isolation
review. Write `PHASE-2-CORE-RESULTS.md` (hard-gate evidence table, named test
evidence, CI evidence, deferred-item backlog reference, doc/ADR corrections
applied, explicit list of what was NOT built). Open a PR; all four CI jobs
(`verify` / `security` / `e2e` / `realtime`) green. **STOP for owner integration
approval** — do not merge automatically, do not tag from an unmerged branch.
After owner-approved integration + push-to-main CI green: annotated tag
**`phase-2-core-complete`** on that green `main` commit; push; verify via the
GitHub API. Do **not** create `phase-2-complete`. **STOP.**
...
| **HG-OUTBOX**          | Atomic mutation + outbox row in one txn; **at-least-once** publication; **`event_id` and `seq` assigned once and immutable** (FC-1); **crash after `XADD` before `dispatched_at`** → duplicate publish carrying the **identical `event_id` + `seq`**, downstream effect applied once, **no event lost**; a persistently failing row is retried forever with bounded backoff, stays observable (`attempts` / `last_error` + lag metrics), and **never blocks another row or another tenant** — **no `outbox_dead_letter` table** (Task 2.4 accepted design; reconciliation in §7 + `PHASE-2-CORE-RESULTS.md`). |
...
| Outbox dispatch                 | integration (pg+redis) + fault injection          | at-least-once; **`event_id` + `seq` assigned once, immutable (FC-1)**; **crash after publish before ack** → dup publish with identical `event_id` + `seq`, single effect; ordering; persistent-failure isolation (bounded backoff, no dead-letter table, one failing row/tenant never blocks others) | HG-OUTBOX    |
| Stream retention (2.8)          | unit + integration (redis)                        | `XTRIM MINID` time floor; old trimmed / new kept; tenant A ≠ tenant B; idempotent repeat; cursor-below-floor → resync contract; within-retention replays; `event_id` / `seq` never rewritten; no `MAXLEN` logic                                                                                      | HG-RT-RESUME |
| Operational visibility (2.8)    | unit + integration (pg + redis)                   | bounded aggregate outbox lag (undispatched / oldest-age / with-failures) + stream growth; O(1) metric cardinality; threshold breach names the worst tenant in a structured log line, never a metric label                                                                                            | —            |
...
| **HG-OUTBOX wording (this doc §2.4 / §4 / §5)** | "poison row → dead-letter" / "dead-letter table" replaced with Task 2.4's **accepted** behaviour: bounded-backoff-forever, never discarded, a backed-off row excluded from its own tenant's selection, per-tenant `try/catch` + advisory locking so one failing row/tenant never blocks another, persistent failure visible via `attempts` / `last_error` + lag metrics. **No `outbox_dead_letter` schema/table is added** (owner decision, 2026-09-05). | 2.8       |
| `REALTIME-PROTOCOL-INPUTS.md`                   | authority banner added — the file is a historical design input; **ADR-0017 is authoritative and governs**; F8/F9 problem analysis kept verbatim, not rewritten                                                                                                                                                                                                                                                                                           | 2.8       |
| `ARCHITECTURE.md §13–14` / ADR-0017 §5          | realtime Stream retention (`XTRIM MINID`, time-based) is **implemented** in Task 2.8: `stream-retention` scheduler queue → worker sweep, `STREAM_RETENTION_MS` / `STREAM_RETENTION_SWEEP_INTERVAL_MS` configurable                                                                                                                                                                                                                                       | 2.8       |
