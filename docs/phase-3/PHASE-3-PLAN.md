...
>    `?at=<ISO>` — an ISO-8601 instant (or bare `YYYY-MM-DD`), default now.
>    **DATE CONTRACT (CHECK 2):** the fiscal `effective_from` / `effective_to`
>    bounds are PostgreSQL `DATE`s; `at` is reduced to its **UTC calendar date**
>    (`toFiscalDate`) before every fiscal query, so resolution is deterministic
>    across timezone offsets AND DB session timezones (`…T00:30+04:00` and the
>    equal `…T20:30Z` resolve byte-identically — no ±1-day drift; no jurisdiction
>    timezone). The civil-date lookups are **`::date`-cast raw SQL** (not a
>    Prisma `Date` filter, which drifts near a `DATE` boundary). `resolved_at`
>    echoes the instant asked about. Country comes ONLY from
>    `company.country_code` (authoritative — never a client value, query param,
>    header, `branch_id` or `pos_terminal_id`; branch has **no** tax state).
>    Reuses `LocalizationService.resolveCompanyCountry` (fail closed:
>    `409 COMPANY_LOCALIZATION_NOT_CONFIGURED` if the company has no country) +
>    `resolveRegimeOn` (`500 TAX_REGIME_NOT_CONFIGURED` no regime on that date /
>    `500 TAX_REGIME_AMBIGUOUS` `> 1` regime row) + `resolveTaxRate` — **for one
>    `(country, category, civil date)` there must be AT MOST ONE applicable
>    `tax_rate`; `> 1` in-force row (overlap of ANY shape — same/different
>    `effective_from`, finite/open-ended) → `500 TAX_RATE_AMBIGUOUS`, fail
>    closed, never pick one** (CHECK 1). Response
...
| 3.9  | `GET /v1/catalog/companies/:companyId/variants/:variantId/tax` (effective category + effective `tax_rate` for `company.country_code` at `?at=`)                                                                  | `catalog:view`                                                                                   | `@ScopedParam({ company: 'companyId' })`     | reads Task 2.7 reference data via `TaxResolutionService` / `LocalizationService`; **`rate_bps` only, NO amount** (D2-8). `?at=` optional ISO-8601 instant / `YYYY-MM-DD` (default now), matched on its **UTC calendar date** (`DATE`-backed fiscal cols, deterministic across offsets — CHECK 2); malformed → `400 INVALID_DATE`; any other query key → `400`. Missing category / regime NONE / no in-force rate → `200` with `reason ∈ NO_CATEGORY_ASSIGNED \| REGIME_NONE \| NO_RATE_FOR_CATEGORY` (never `422`); a configured `0` is `rate_bps: 0, reason: null`. `country_code` is `company.country_code` only — never client / branch / POS. `409 COMPANY_LOCALIZATION_NOT_CONFIGURED` (no country); `500 TAX_REGIME_NOT_CONFIGURED` (no regime on that date) / `500 TAX_RATE_AMBIGUOUS` (**> 1 in-force rate, any overlap shape** — CHECK 1) fail closed. No audit row.                           |
