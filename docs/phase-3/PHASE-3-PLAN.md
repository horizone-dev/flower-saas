...
| **3.10** | Business-Type **capability**-template re-apply + catalog transactional outbox events + company-scoped realtime auth                                                                                                                           | re-apply txn (capability rows only — owner D-7) + `catalog.*` outbox events + **one additive nullable `outbox.company_id`** (owner D-1, supersedes the old "no migration" note) | **yes (1 additive nullable column)** | 3.1–3.9       |
| **3.11** | Phase 3a verification pass + `PHASE-3A-RESULTS.md` + `phase-3a-catalog-complete` tag — **verification complete, see [`PHASE-3A-RESULTS.md`](PHASE-3A-RESULTS.md); tag created only after this task's PR merges and push-to-main CI is green** | —                                                                                                                                                                               |                **no**                | 3.1–3.10      |
...
separation; tax is a thin catalog annotation + a read service (3.9); the catalog
transactional outbox events + company-scoped realtime auth + the Business-Type
**capability**-preset re-apply land together (3.10, one additive nullable
`outbox.company_id` column); verification last (3.11).
...
| ~~`template_payload`~~ | **NOT ADDED (owner D-7, 2026-09-09).** The shipped Business-Type template mechanism is **capability-only** — the normalized `business_type_template_capability` table holds a `{ capability_key, enabled, config }` row set and nothing else. The "suggested categories / attribute templates / variant templates / UOM templates" concept was reassigned by `PHASE-3.1-CAPABILITY-SPEC.md` §A.1 to Tasks 3.2 / 3.3 / 3.4 / 3.6, which shipped **without** building any `business_type_template_category` / `_attribute` / `_uom` seed table. If richer catalog templates are ever wanted they are a **new, separately-approved task** — Task 3.10 does NOT implement them. |
...
> **Corrections (Task 3.5 implementation, owner-locked 2026-09-07).** Five
> changes to the sketch above:
>
> 1. Uniqueness is `UNIQUE (tenant_id, value)` — spanning every `code_type` and
>    both statuses — not `(tenant_id, code_type, value)`. A deactivated value is
>    never reused for another target. Plus partial uniques: ≤ 1 `ACTIVE` SKU and
>    ≤ 1 `ACTIVE` QR per target; many `ACTIVE` BARCODE rows allowed.
> 2. `target_kind` is `VARIANT`-only in the DB CHECK. `INVENTORY_ITEM` is a
>    documented reserved value, not legal (no `inventory_item` table — D2-11);
>    Phase 5 widens the CHECK when real RI to `inventory_item` exists.
> 3. `pack_uom_code` / `pack_qty` move to Task 3.6 — a UOM code is not stored
>    before the `uom` registry that validates it exists.
> 4. SKU stays single-sourced in `item_identifier` (`code_type = SKU`; no
>    `variant.sku` — Task 3.4 L-1); optional, manual, canonical.
> 5. RI is a server-derived `target_variant_id` GENERATED column + composite FK
>    to `variant(tenant_id, id)` `ON DELETE RESTRICT` (never CASCADE). No
>    `version` column — identity is immutable (no `PUT`; `ACTIVE ↔ INACTIVE`).

| Table             | RLS          | Unique                                                          | Notes                                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------- | ------------ | --------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `item_identifier` | ENABLE+FORCE | **`(tenant_id, value)`** + partial `ACTIVE` SKU / QR per target | `target_kind text CHECK (∈ VARIANT)` (`INVENTORY_ITEM` reserved, not legal); `target_id uuid` + server-derived `target_variant_id` GENERATED + composite FK `ON DELETE RESTRICT`; `code_type text CHECK (∈ SKU, BARCODE, QR)`; `value text` (bounded / trimmed / control-char-free CHECK); `status` (`ACTIVE`/`INACTIVE`). No pack / price / stock / company / branch / version column. |

### C.7 Task 3.6 — UOM configuration + conversions (tenant-scoped)

> **Corrections (Task 3.6 implementation, owner-locked 2026-09-08).** The sketch
> below is refined by the frozen Task 3.6 scope:
>
> 1. **Canonical UOM code** — one convention, lowercase, `^[a-z][a-z0-9._-]{0,31}$`
>    (**no `/`** — a code is a REST path segment). `canonicalUomCode` (trim +
>    lowercase) + `BUILTIN_UOMS` / `isBuiltinUom` are added to `@flower/uom`;
>    built-in-shadow detection runs after canonicalization.
> 2. **`uom` semantics are immutable** after create (`code` / `family` /
>    `perBaseNum` / `perBaseDen` / `maxDecimals`) — `PUT` edits only `name_en` /
>    `name_ar`. Hard-delete only while completely unreferenced (`FOR UPDATE` +
>    dependency check; `409 UOM_IN_USE`). `EACH ⇒ perBase 1/1` CHECK.
> 3. **`variant.base_uom_code` stays NULLABLE** — **no backfill**, **no NOT NULL**
>    in Task 3.6 (`piece` is not a safe universal default). Lifecycle: `NULL →
valid` is a one-time init allowed even on a legacy ACTIVE variant with zero
>    conversions / zero pack identifiers; `non-NULL → different` only while DRAFT
>    with the same zero-dependency rule; never `→ NULL`. From Task 3.6,
>    **STOCKED / BOM** variant activation/reactivation requires `base_uom_code`;
>    **CUSTOM** does not.
> 4. **`uom_conversion` is base-anchored (P2)** — no stored edge graph, no
>    traversal. VARIANT rows anchor to the variant base (partial unique
>    `(tenant, scope_id, from)`); PRODUCT rows carry an explicit `to` and may
>    repeat `from` with different `to` (partial unique `(tenant, scope_id, from,
to)`). A PRODUCT row is inherited by a variant only when `to == variant
base`; a matching VARIANT row overrides it. An explicit row for a pair the
>    registry already resolves by same-family `perBase` math (built-in **or**
>    tenant physical/COUNT unit) → `422 UOM_CONVERSION_REDUNDANT`. Composite
>    tenant-safe FKs on generated `scope_variant_id` / `scope_product_id`,
>    `ON DELETE CASCADE`.
> 5. **`item_identifier` pack snapshot** — additive nullable `pack_uom_code` /
>    `pack_qty` / `pack_base_qty` (all-or-none CHECK; SKU-forbidden CHECK). BARCODE
>    / QR only, needs `identifiers.barcode_qr` **and** `multi_uom`. `pack_base_qty`
>    is computed **exactly once** with `UomRegistry.convertExact` (exact-or-reject
>    — `422 IDENTIFIER_PACK_BASE_INEXACT`, never `HALF_UP`) and is **immutable** —
>    a later conversion edit never rewrites it; a scan returns the frozen value,
>    never a live recompute. New `@flower/uom` `divRoundExact` / `InexactError` /
>    `Quantity.scaleByExact` / `UomRegistry.convertExact` / `InexactConversionError`.
> 6. **No DB FK for textual UOM-code references** (`base_uom_code`, conversion
>    `from`/`to`, `pack_uom_code`) — built-ins have no row (OD-5). Concurrency is
>    closed by row locks: a reference write `SELECT … FOR KEY SHARE`s every
>    tenant-custom code (canonicalized / de-duped / ascending); a custom-UOM
>    `DELETE` `SELECT … FOR UPDATE`s the target then checks dependencies. `uom`
>    is the terminal lock — no cycle with `product`/`variant`/`item_identifier`.
> 7. **No new permission / role-template change** (`catalog:manage` /
>    `variants:manage` reused). `multi_uom` (an existing capability key) gates all
>    tenant-custom-UOM + conversion + pack-metadata writes; reads and built-in
>    base-UOM assignment are never gated; disabling it never hides or destroys
>    existing data.
...
> **Corrections (Task 3.7 implementation, owner-locked 2026-09-08 — FINAL scope
> freeze rev.3).** The sketch below is superseded by the frozen Task 3.7 scope:
>
> 1. **NO price gate on activation (D-1).** `product` / `variant` activation is
>    **not** modified. `ACTIVE` = catalog-ready; a variant may be `ACTIVE` with
>    zero price rows. Sellability (Phase 3b) = `ACTIVE` + a resolvable
>    company/UOM price + (Task 3.8) branch availability + POS rules. The lines
>    below reading "a variant cannot be `ACTIVE` for sale by a company until…"
>    and (§D row 3.2) "activate gated: … a base-UOM company price exists…" are
>    **withdrawn**.
> 2. **The base-UOM price is NOT mandatory (D-4).** A company may price **any
>    single valid reachable selling UOM** — e.g. `box` only, with `piece` left
>    explicitly unpriced. A tier's price is **never** derived from another
>    (`base × factor` / `box ÷ factor`) — ADR-0018 §5. Reachability is validated
>    for legitimacy only (base, or resolvable to base via the Task 3.6 effective
>    conversion model) → else `422 PRICE_UOM_UNREACHABLE`.
> 3. **Dedicated aggregate version (D-7).** A new `company_variant_price_set`
>    table (`id` / `tenant_id` / `company_id` / `variant_id` / `version` /
>    timestamps, `UNIQUE (tenant_id, company_id, variant_id)`) owns the
>    concurrency `version`. **Never `variant.version`.** `company_variant_uom_price`
>    FKs to it by the `(tenant_id, company_id, variant_id)` composite,
>    `ON DELETE CASCADE`; the price row carries **no `version` column**. The
>    replace-set `PUT` is guarded by `If-Match: "<price-set version>"` (`"0"` =
>    create). First successful `PUT` → version **1**. `PUT []` deletes the rows,
>    **keeps** the aggregate, bumps the version. Absent aggregate → `GET`
>    `version: 0`, `ETag: "0"`.
> 4. **Currency code + exponent are DB-enforced (Inv-3).** `sell_currency_code`
>    **must** equal `company.default_currency` — composite FK
>    `(tenant_id, company_id, sell_currency_code) → company(tenant_id, id,
default_currency)` `ON UPDATE RESTRICT`. `(sell_currency_code,
sell_currency_exponent)` **must** be an authoritative pair — composite FK to
>    `currency(code, exponent)` `ON UPDATE RESTRICT`. Neither uses
>    `ON UPDATE CASCADE`. Additive FK-target `UNIQUE`s on `company` (`(tenant_id,
id)`, `(tenant_id, id, default_currency)`) and `currency` (`(code,
exponent)`) — no column change, no `currency` business-model change.
> 5. **Task 3.7 resolution is COMPANY-ONLY (D-10).** `PriceResolutionService`
>    takes `(company_id, variant_id, uom_code)` — **no `branch_id`**. The
>    `/resolve` query is strict: `?uomCode=` only; a `branchId` param → **400**.
>    Task 3.8 adds `branchId` + the branch-override arm + branch precedence
>    together. The precedence block below (branch → company → no-price) is the
>    Task-3.8 target shape; in Task 3.7 the branch arm does not exist.
> 6. **`resolve` route** is `GET …/prices/resolve?uomCode=…` (not `?branchId=&uom=`).
>    A missing price is a normal **`200 { price: null, reason }`** (D-13), never a
>    `422` for absence. `reason ∈ NO_PRICE_SET | UOM_NOT_PRICED | UOM_UNRESOLVABLE`.
> 7. **SELL price only in the wire contract (D-6).** `purchase_*` stays a
>    nullable, **API-absent**, unconsumed Phase-5 foundation — the `PUT` body,
>    `GET`, `/resolve` and the audit payload never carry it (a `purchase` field in
>    the body → **400**). Sell price is tax-**exclusive / net** (D-2), `> 0`
>    (D-8). Task 3.7 computes no tax.
> 8. **`pricing:manage`** (already in `PERMISSIONS.catalog`) is registered by the
>    Task 3.7 migration + backfilled to `owner` / `admin` (not `manager`). **Not
>    step-up** (D-9). **No `company_pricing` catalog capability** (D-12) — company
>    pricing is foundational.
> 9. **Audit only (D-14).** One `catalog.company_price_changed` row (resource =
>    the `company_variant_price_set` aggregate) per replace-set mutation.
>    **No outbox / realtime** — `catalog.company.price_changed` (§H) is emitted by
>    **Task 3.10**.
> 10. **Additive Task-3.6 guard rules.** A custom `uom` cannot be hard-deleted
>     while any `company_variant_uom_price` row references its code
>     (`409 UOM_IN_USE`, even when that row is `resolvable: false`). A variant's
>     base UOM cannot change while any company price row exists for it, across any
>     company (`409 VARIANT_BASE_UOM_LOCKED`) — `PUT []` for every pricing company
>     first, then the Task-3.6 base-UOM rules apply again. Deleting a
>     `uom_conversion` that orphans a priced UOM is **allowed** — the price row
>     stays, `GET` reports `resolvable: false`, `/resolve` → `UOM_UNRESOLVABLE`.
>
> Branch price override + branch availability remain **Task 3.8**; tax category
> and rate resolution remain **Task 3.9**; discounts / promotions / price lists /
> customer groups / effective dates remain out of Phase 3a scope.
...
> **Corrections (Task 3.8 implementation, owner-locked — FINAL scope freeze rev.5,
> BD-1…BD-17 + Corrections 1–5 / A–H).** The sketch below is superseded by the
> frozen Task 3.8 scope:
>
> 1. **THREE tables, not two.** `branch_variant_price_set` (`id` / `tenant_id` /
>    `company_id` / `branch_id` / `variant_id` / **`version`** / timestamps,
>    `UNIQUE (tenant_id, company_id, branch_id, variant_id)`) is the branch-price
>    concurrency aggregate — it owns the `version` (BD-3, **never** `variant.version`),
>    is **independently monotonic** (created at 1, only ever incremented, **never
>    reset, never deleted**), and **may validly exist with zero override rows and
>    zero company pricing** (a first `PUT { prices: [] } If-Match "0"` creates it —
>    Correction 2). It has **NO FK to `company_variant_price_set`**.
>    `branch_variant_uom_price` carries **NO `version`** column and **NO
>    `purchase_*`** (SELL-only — BD-9); it has **NO row-level FK to
>    `company_variant_uom_price`**.
> 2. **NO `DELETE …/prices` endpoint (BD-4).** `PUT { prices: [] }` is the only
>    clear operation (hard-deletes the rows, retains the aggregate, bumps the
>    version). The "Deletable (audited) — a pure override" note applies to the
>    override **rows**, not the aggregate. The version is never reset to 0 (ABA
>    prevention).
> 3. **BD-1 — a branch override ROW requires a matching `company_variant_uom_price`
>    row** for `(company, variant, uom_code)` → else `422 BRANCH_PRICE_NO_COMPANY_PRICE`.
>    This does **not** constrain the branch aggregate. Enforced **in the service**
>    (no row-level FK — Task 3.7 replace-sets delete/recreate).
> 4. **Company ↔ branch price-row integrity is synchronized on the Task 3.7
>    `company_variant_price_set` aggregate as the SHARED LOCK** (branch write:
>    `FOR SHARE`; Task 3.7 company `replace()`: `FOR UPDATE`). The company
>    `replace()` gains a guard — while it holds `company_variant_price_set FOR
UPDATE`, a company UOM price that any branch of the company overrides cannot
>    be removed → `409 COMPANY_PRICE_HAS_BRANCH_OVERRIDE` (payload = blocked UOM
>    codes + a row count, **never** a sibling `branch_id`). This guard runs
>    **regardless of the `branch_pricing` capability state**.
> 5. **`requested_branch_id` (the `:branchId` path segment) is the resource
>    selector**, authorized by `@ScopedParam({ branch: 'branchId' })` +
>    `PolicyEngine` step 9 against `ctx.branchScope` + the per-branch overlay
>    **before** any business access. `company_id` is **derived** from the
>    authorized branch — no `:companyId` path segment, never a client value.
>    `single_branch_id` (→ the `app.branch_id` RLS GUC) is a **narrowing context
>    only**, never the resource selector, and is **never** derived from
>    `pos_terminal_id`. A branch → company pre-read is unlocked and does not
>    participate in lock ordering; `branch.company_id` is revalidated under the
>    branch lock (`409 BRANCH_COMPANY_CHANGED` on a concurrent reassignment —
>    impossible in the current domain, defended).
> 6. **Branch-GUC RLS** — all three tables: `ENABLE` + `FORCE` + a policy that
>    narrows to `app.branch_id` when set, else tenant-wide. The **three narrow
>    cross-branch integrity read helpers** (`branch-price-integrity.repo.ts` —
>    `findTenantWideBlockedCompanyPriceUoms` / `countTenantWideBranchPriceUomDependencies`
>    / `countTenantWideBranchPriceVariantDependencies`) neutralize **ONLY**
>    `app.branch_id` for **exactly one** read-only `SELECT` (tenant-predicated),
>    then restore it. **NO generic `withTenantWideBranchIntegrity(tx, fn)`
>    callback** (Correction F); each call is a standalone `await`, never inside a
>    `Promise.all`. The custom-UOM delete guard and the base-UOM change guard use
>    helpers #2/#3 AFTER `uom FOR UPDATE` / `variant FOR UPDATE` is held
>    (Correction G — no pre-lock count, no TOCTOU). Task 3.8 adds **no new DB
>    role**, no `BYPASSRLS`, no `runPlatform` / `runDispatcher` in a request flow.
> 7. **Branch override currency == `company.default_currency`** — DB backstops
>    `(tenant_id, company_id, override_currency_code) → company(tenant_id, id,
default_currency)` `ON UPDATE RESTRICT` and `(override_currency_code,
override_currency_exponent) → currency(code, exponent)` `ON UPDATE RESTRICT`
>    (never `CASCADE`). Structural money → `400`; semantic (unknown currency /
>    wrong authoritative exponent / ≤ 0 / ≠ company default) → `422 BRANCH_PRICE_*`.
> 8. **`branch_price:manage` gates BOTH surfaces; the `branch_pricing` capability
>    gates EVERY branch WRITE** — branch price writes AND branch availability
>    writes (owner ruling 2026-09-09, superseding the earlier "availability
>    ungated" wording). Reads (`GET …/prices`, `…/prices/resolve`,
>    `…/availability`, `…/catalog`) are **never** capability-gated. The gate is
>    the application-layer `assertEnabled(...)` in `BranchPricingService`, inside
>    the handler — on a `409 CAPABILITY_NOT_ENABLED` the idempotency interceptor
>    releases the claim: no business mutation, no audit row, a later retry
>    re-executes. `branch_price:manage` → built-in `owner` / `admin` only (not
>    `manager` by default); not step-up. No separate `branch_availability`
>    permission or capability.
> 9. **`branch_variant_availability` stores ONLY `available boolean`** (BD-7 — no
>    `pos_visible` / `web_visible` / `display_order` / `reason` / `note` /
>    quantity). Absence ⇒ `available: true, explicit: false`. It is **independent
>    of pricing** (BD-6 — no "company-owned variant" concept; validation needs only
>    that the branch is in scope and the variant exists in the tenant). It never
>    gates activation and never alters price resolution. Bulk `PUT` is
>    **declarative, atomic (all-or-nothing), `Idempotency-Key`, no `If-Match`,
>    no version**. Structural DTO validation → `400`; the semantic checks run
>    AFTER the parse as explicit typed domain errors — duplicate `variant_id` →
>    `422 BRANCH_AVAILABILITY_DUPLICATE_VARIANT` (a `DomainError`, **not** a Zod
>    refinement — Correction H); non-ascending order → `400`; unknown tenant
>    variant → `422 BRANCH_AVAILABILITY_VARIANT_NOT_FOUND`. Idempotency is
>    **principal-scoped and branch-bound** (the existing store identity
>    `(tenant, scope, principal, key)` + the `pathParams.branchId` fingerprint —
>    unchanged storage model, Correction D). A replay writes no second audit row.
> 10. **`GET …/branches/:branchId/availability?variantId=`** — an unknown /
>     cross-tenant `variant_id` → `404` (no leakage, Correction E); an existing
>     tenant variant with no explicit row → `200 { available: true, explicit: false }`.
> 11. **API — the dedicated branch resolver** `GET
…/branches/:branchId/variants/:variantId/prices/resolve?uomCode=` (strict:
>     an extra `branchId` → `400`; malformed `uomCode` → `422 UOM_INVALID_CODE`
>     before any DB access). The **Task 3.7 company `/resolve` is unchanged** and
>     still rejects `branchId` with `400` (BD-8). Precedence: branch override →
>     company default → `{ price: null, reason }`; `source ∈ BRANCH | COMPANY |
null`; `reason ∈ NO_PRICE_SET | UOM_NOT_PRICED | UOM_UNRESOLVABLE | null`;
>     the response also carries `branchAvailable` (informational — never changes
>     `price`, BD-15/BD-16).
> 12. **Effective catalog** `GET …/branches/:branchId/catalog` — cursor
>     paginated; one entry **per variant for which the branch's company has ≥ 1
>     current `company_variant_uom_price` row** (there is no "company-owned
>     variant" concept — catalog variants are tenant-level); branch overrides
>     replace the matching company UOM tiers. Returns only `{ variantId,
productId, available, prices: [{ uomCode, sell, source, resolvable }] }` —
>     no product names / categories / attributes / identifiers / media /
>     inventory (BD-14). The candidate page is a DB-guaranteed
>     `SELECT DISTINCT "variantId" … WHERE "variantId" > :cursor ORDER BY
"variantId" ASC LIMIT :limit + 1` — **unique-variant** pagination, not
>     price-row pagination (a multi-UOM variant consumes exactly one slot). The
>     `limit + 1` lookahead sets `hasMore`: the first `limit` ids are emitted,
>     `nextCursor` = the **last emitted** `variantId` when `hasMore` else
>     `null`. `nextCursor: null` ⇒ the last page — **no trailing empty request**.
> 13. **Audit only** (`catalog.branch_price_changed` — resource = the
>     `branch_variant_price_set` aggregate; `catalog.branch_availability_changed`
>     — `resourceType: 'branch'`, `resourceId` = the authorized `branch_id`,
>     Correction 4). Both `security: false`; **no `security_event` change**; **no
>     outbox / realtime** — `catalog.branch.price_changed` / `.availability_changed`
>     (§F) are **emitted from Task 3.10**.
> 14. **`variants` in a branch price row: NO `version` column** — the aggregate
>     owns concurrency; **override rows are hard-deletable** via `PUT []`.
>     `§I.2` is now **locked** (BD-1).
>
> One additive, forward-only migration `20260912120000_catalog_branch_pricing`
>
> - two additive FK-target `UNIQUE`s on the shipped `branch` table
>   (`(tenant_id, id)`, `(tenant_id, company_id, id)` — no column change, precedent:
>   Task 3.7 on `company` / `currency`). Tax category + rate resolution remain
>   **Task 3.9**; discounts / promotions / price lists / customer groups / effective
>   dates remain out of Phase 3a.
...
>    country has no VAT regime on the requested civil date, e.g. QA / KW) ·
>    `NO_RATE_FOR_CATEGORY` (VAT regime but no in-force `tax_rate` row for the
>    resolved category). A configured
...
>    (`catalog:view`, `@ScopedParam({ company: 'companyId' })`), **required**
>    `?date=YYYY-MM-DD`.
>    **DATE CONTRACT (owner freeze — CHECK 2):** FISCAL RESOLUTION DATE = a
>    `YYYY-MM-DD` **civil calendar date**. NOT an instant, NOT UTC-normalized,
>    NOT company/branch/POS-timezone-converted. The fiscal `effective_from` /
>    `effective_to` bounds are PostgreSQL `DATE`; the `date` string is passed
>    verbatim to **`::date`-cast raw SQL** (`"effectiveFrom" <= $date::date AND
("effectiveTo" IS NULL OR "effectiveTo" >= $date::date)` — pure `DATE` vs
>    `DATE`, DB-session-timezone immune) for BOTH `tax_rate` and
>    `country_tax_config`; `effective_to` is **inclusive**. No JavaScript `Date`
>    is ever used to pick the effective calendar day. Strict validation: a real
>    `YYYY-MM-DD` only — a value carrying a time or `+HH:MM` / `Z` offset, a
>    US-format date, a single-digit month/day, or an impossible date
>    (`2026-02-30`) → `400 INVALID_DATE`; a missing `date` or unknown query key →
>    `400 VALIDATION_FAILED`. An ISO timestamp is NEVER silently truncated.
>    `date` is REQUIRED because there is no authoritative company/country fiscal
>    timezone field in the current model (only `branch.timezone`, which §11
>    forbids as a tax-authority source) — an explicit civil date is deterministic
>    and safe. Country comes ONLY from `company.country_code` (authoritative —
>    never a client value, query param, header, `branch_id` or `pos_terminal_id`;
>    branch has **no** tax state). Reuses
>    `LocalizationService.resolveCompanyCountry` (fail closed:
>    `409 COMPANY_LOCALIZATION_NOT_CONFIGURED` if the company has no country) +
>    `resolveRegimeOn(country, date)` (`500 TAX_REGIME_NOT_CONFIGURED` no regime
>    on that date / `500 TAX_REGIME_AMBIGUOUS` `> 1` regime row) +
>    `resolveTaxRate(country, key, date)` — **for one `(country, category, civil
date)` there must be AT MOST ONE applicable `tax_rate`; `> 1` in-force row
>    (overlap of ANY shape — same/different `effective_from`, finite/open-ended)
>    → `500 TAX_RATE_AMBIGUOUS`, fail closed, never pick one** (CHECK 1). Response
...
effective_to, resolved_date, reason }` — `resolved_date` echoes the requested
>    `YYYY-MM-DD` (no timestamp, no timezone). Missing category / missing rate /
...
- **Not every Task 3.x has a migration** — 3.11 does not; **Task 3.10 has exactly
  one additive, forward-only, nullable-column migration** (`outbox.company_id`,
  owner D-1 — this supersedes the earlier planning note that Task 3.10 would be
  migration-free; D2-12 explicitly permits an additive nullable column on a
  shipped table).
...
| 3.5  | `POST /identifiers` · `GET /identifiers?value=…` (resolve) / `?targetKind=VARIANT&targetId=…` (list) · `DELETE …/:id` · `POST …/:id/reactivate`                                                                  | `identifiers:manage` / `catalog:view`                                                            | tenant                                       | Immutable identity — no `PUT` / `If-Match`. `POST` create + `reactivate` → `Idempotency-Key`; `DELETE` → plain. BARCODE / QR also need `identifiers.barcode_qr`. `409` on a duplicate value or active SKU / QR; `422` on `INVENTORY_ITEM`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 3.6  | `POST/PUT/DELETE/GET /v1/catalog/uoms[/:code]`                                                                                                                                                                   | `catalog:manage` / `catalog:view`                                                                | tenant                                       | POST → `Idempotency-Key`; `PUT` (names only — semantic fields immutable) → `If-Match: <uom.version>`; `DELETE` → `If-Match`, `409 UOM_IN_USE` while referenced. Every write also needs `multi_uom`. `422 UOM_INVALID_DEFINITION` / `422 UOM_BUILTIN_SHADOW`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 3.6  | `PUT /v1/catalog/variants/:id/base-uom` · `GET/PUT /v1/catalog/variants/:id/conversions` (effective view / replace-set) · `GET/PUT /v1/catalog/products/:id/conversions` (stored rows incl. inert / replace-set) | `variants:manage` / `catalog:view`                                                               | tenant                                       | parent `If-Match: <variant\|product version>`. base-UOM: `multi_uom` only for a tenant-custom code (a built-in base needs none). conversion writes: `multi_uom` always. `422` invalid ratio / unregistered unit / `UOM_CONVERSION_REDUNDANT`; `409 VARIANT_BASE_UOM_LOCKED` / `VARIANT_BASE_UOM_REQUIRED`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| 3.9  | `GET /v1/catalog/companies/:companyId/variants/:variantId/tax` (effective category + effective `tax_rate` for `company.country_code` on `?date=`)                                                                | `catalog:view`                                                                                   | `@ScopedParam({ company: 'companyId' })`     | reads Task 2.7 reference data via `TaxResolutionService` / `LocalizationService`; **`rate_bps` only, NO amount** (D2-8). **`?date=YYYY-MM-DD` REQUIRED** — a civil calendar date, NO time / NO timezone / NO ISO instant; matched by pure `::date`-cast SQL (`DATE` vs `DATE`, `effective_to` inclusive); a timezone-bearing timestamp, US-format, single-digit, or impossible date → `400 INVALID_DATE`; missing `date` / any other query key → `400 VALIDATION_FAILED`; never silently truncated. No JS `Date`, no instant→date reduction, no jurisdiction/company/branch/POS timezone. Missing category / regime NONE / no in-force rate → `200` with `reason ∈ NO_CATEGORY_ASSIGNED \| REGIME_NONE \| NO_RATE_FOR_CATEGORY` (never `422`); a configured `0` is `rate_bps: 0, reason: null`. Response `resolved_date` = the requested `YYYY-MM-DD`. `country_code` is `company.country_code` only — never client / branch / POS. `409 COMPANY_LOCALIZATION_NOT_CONFIGURED` (no country); `500 TAX_REGIME_NOT_CONFIGURED` (no regime on that date) / `500 TAX_REGIME_AMBIGUOUS` / `500 TAX_RATE_AMBIGUOUS` (**> 1 in-force row, any overlap shape** — CHECK 1) fail closed. No audit row.                                                                                                                                                                                                                                                                   |
...
| 3.10 | `POST /v1/platform/tenants/:tenantId/apply-business-type-template`                                                                                                                                               | `platform:catalog_capability:manage` (platform, **fresh step-up**)                               | —                                            | **BOTH** `Idempotency-Key` (required — 400 `IDEMPOTENCY_KEY_MISSING` / `_INVALID`; a Redis-guard replay returns the stored response, no re-execution) **AND** `If-Match: "<catalogCapabilityVersion>"` (required — 428 `PRECONDITION_REQUIRED` missing, 409 `CATALOG_CAPABILITY_VERSION_CONFLICT` stale) — owner D-3. Body `{ templateKey, mode?: 'merge' \| 'replace' }` (`.strict()`, `mode` default `merge`). `templateKey` MAY differ from the tenant's current `businessTypeKey` (owner D-4) — a successful logical apply re-stamps `businessTypeKey` / `businessTypeAppliedVersion` / `businessTypeAppliedAt`. ONE `runPlatform` txn: lock tenant `FOR UPDATE` → verify version → resolve target template (422 `UNKNOWN_BUSINESS_TYPE` / `BUSINESS_TYPE_NOT_ACTIVE`) → merge/replace **capability rows only** per `PHASE-3.1-CAPABILITY-SPEC.md` §J (never deletes a key absent from the target; `merge` skips `MANUAL` rows; `replace` overwrites any + returns provenance to `TEMPLATE` + clears `overriddenAt`) → if a logical mutation occurred: `+1` aggregate version once + ONE bounded `catalog.template_applied` audit row (`{ mode, fromTemplateKey, fromTemplateVersion, toTemplateKey, toTemplateVersion, changedCapabilityKeys }`). Exact semantic no-op → no version bump, no audit, return current state + ETag. **NO outbox / realtime** (owner D-2). **NO catalog-entity / price / inventory** (owner D-7). ETag = `aggregateVersion`. |
...
## F. Event / realtime matrix — **EMITTED from Task 3.10 (owner D-1 / D-2 / D-5)**

Publisher: the catalog repository writes an `outbox` row **in the same DB
transaction** as the domain mutation + its audit row (existing `OutboxWriter`);
downstream is the unchanged Task 2.4→2.5→2.6 pipeline. Envelope = the ADR-0017 §3
fields **+ the additive `company_id`** (Amendment A1) — **never the payload**.
Events are tenant-isolated, and authorised **cumulatively** (tenant + company +
branch) by the gateway, `event_id`-deduped, cursor-resumable on both the live and
the replay path.

| Event `type`                          | When                                                                                                                                                                                | `company_id` / `branch_id`               | `resource_version`                          | payload hint (bounded — owner D-6)               | Volume |
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | ------------------------------------------- | ------------------------------------------------ | ------ |
| `catalog.company.price_changed`       | every successful `company_variant_uom_price` replace-set                                                                                                                            | `company` / `null`                       | `company_variant_price_set.version`         | `{ variantId, changedUomCodes }`                 | low    |
| `catalog.branch.price_changed`        | every successful `branch_variant_uom_price` replace-set                                                                                                                             | `branch.companyId` / `branch` (**both**) | `branch_variant_price_set.version`          | `{ variantId, changedUomCodes }`                 | low    |
| `catalog.branch.availability_changed` | every successful branch-availability bulk set                                                                                                                                       | `branch.companyId` / `branch` (**both**) | `null` (no aggregate version; not invented) | `{ variantIds }`                                 | low    |
| `catalog.product.status_changed`      | a product status transition that **changes consumer visibility** (owner D-5 — `ACTIVE` on one side: `DRAFT→ACTIVE`, `ARCHIVED→ACTIVE`, `ACTIVE→ARCHIVED`; **not** `DRAFT→ARCHIVED`) | `null` / `null`                          | `product.version` (after)                   | `{ productId, fromStatus, toStatus }`            | low    |
| `catalog.variant.status_changed`      | a variant status transition that changes consumer visibility (same rule)                                                                                                            | `null` / `null`                          | `variant.version` (after)                   | `{ variantId, productId, fromStatus, toStatus }` | low    |

**Not published** (POS/Owner refetch on demand — owner D-2): `DRAFT` field edits,
category-tree edits, product-type edits, attribute-definition/option edits,
`product_attributes_changed`, identifier add/remove/reactivate, UOM / conversion
/ base-UOM edits, product/variant **tax-category** assignment, capability `PATCH`,
**and the Business-Type template re-apply** (a Super-Admin action — the Owner UI
refetches; owner §14 / D-2). Every one of these keeps its **audit** row
unchanged; only the 5 events above **also** get an `outbox` row. `catalog.*` is
NOT a `security_event` prefix (no `security_event` view change). A payload hint is
NEVER a `Money` amount, a full row, or PII; it is NEVER copied into the realtime
envelope by the dispatcher (owner D-6).
**No** new topic / channel / transport. **Open Decision I.3 is RESOLVED (owner
D-1, 2026-09-09)** by option (a): the ADR-0017 envelope gains an additive
optional `company_id` (Amendment A1), `outbox` gains an additive nullable
`companyId UUID` column (migration `20260914120000_catalog_realtime_company_scope`),
and the gateway's `isAuthorized` gains a cumulative company-scope predicate
(`session.companyScope` must be `'ALL'` or include `company_id`). A
`catalog.company.*` event is delivered only to sockets whose session
`companyScope` covers that company; a `catalog.branch.*` event carries BOTH
`company_id` and `branch_id`, and a socket scoped to the branch but NOT the
company is still denied. Company filtering is applied identically on the live and
the resume/replay path (`hub.ts` re-authorizes every replayed entry against the
current session).
...
| **HG3-BRANCH-ISOLATION**         | **The branch-isolation probe suite is introduced by Task 3.8 (`apps/api/src/probes/branch-isolation.probe.test.ts`), build-blocking** — a Dubai-scoped user cannot read/write a Sharjah branch's availability or price override; a multi-branch user reaches only its granted branches; **the requested branch is the authorized `:branchId` path selector — never `singleBranchId`, never `posTerminalId`** (a session bound to a Sharjah POS terminal but scoped to Dubai confers no Sharjah reach); the single-branch `app.branch_id` RLS GUC physically returns 0 rows for a sibling branch; the per-branch permission overlay narrows correctly. **Plus a company-isolation check** (company A user vs company B branch pricing) and a **cross-branch integrity** check — a Dubai-only caller's company-price removal / custom-UOM delete / base-UOM change is still blocked by a Sharjah override (the three narrow tenant-wide read helpers are not blinded), the error leaks **no** sibling `branch_id`, the helpers stay tenant-isolated, and the `app.branch_id` GUC is restored afterwards. `flower_app` stays `NOBYPASSRLS`; Task 3.8 adds no new DB role. |
...
| ~~I.2~~ | **LOCKED (Task 3.8 / BD-1).** A branch override **row** requires a matching `company_variant_uom_price` row for `(company, variant, uom_code)` → else `422 BRANCH_PRICE_NO_COMPANY_PRICE`, service-enforced under the shared `company_variant_price_set` lock. This does **not** constrain the branch price-set **aggregate**, which may exist with zero rows and zero company pricing (a first `PUT { prices: [] } If-Match "0"` creates it).                                                                                                |
| ~~I.3~~ | **RESOLVED (owner D-1, 2026-09-09).** The gateway had only tenant + branch filtering. Task 3.10 adds an **additive optional `company_id`** to the ADR-0017 §3 envelope (Amendment A1), an **additive nullable `outbox.companyId UUID`** column, and a **cumulative company-scope predicate** to `isAuthorized` (`session.companyScope` `'ALL'` or includes it). `catalog.branch.*` events carry BOTH `company_id` + `branch_id` (defence in depth). Applied identically to live + replay. A contained gateway change, not a new architecture. | —                                                                                                                                                                                                                           |
