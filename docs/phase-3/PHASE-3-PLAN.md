...
**Task 3.7 emits NO outbox / realtime event** (it writes the
`catalog.company_price_changed` **audit** row only); `catalog.company.price_changed`
above is _defined_ here but _emitted_ from Task 3.10.
**Task 3.8 likewise emits NO outbox / realtime event** — it writes the
`catalog.branch_price_changed` / `catalog.branch_availability_changed` **audit**
rows only (`security: false`, no `security_event` change); `catalog.branch.price_changed`
/ `catalog.branch.availability_changed` above are _defined_ here but _emitted_
from Task 3.10 (each co-writes an `outbox` row, `branch_id` = the affected branch,
